...
% implement Gauss-Siedel iterative method
...
% The Gauss-Siedel method does not converge since its spectral radius (p(T) = 2) is >= 1.
...
if p >= 1
    disp(sprintf("The Gauss-Siedel method does not converge since its spectral radius (p(T) = %g) is >= 1.\n",p))
else
    disp(sprintf("The Gauss-Siedel method converges because its spectral radius (p(T) = %g) is < 1.\n",p))
end